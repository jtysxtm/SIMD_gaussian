...
void gaussian_elimination(float A[][NUM], int n)
{
    ...
    for (int k = 0; k < NUM; k++) {
        ...
        for (j = k + 1; j + 4 <= NUM; j += 4) {
            __m128 va = _mm_loadu_ps(A[k] + j); // 把A[k][j]~A[k][j+7]装入一个SSE变量va中
            va = _mm_div_ps(va, v1); // 执行va/v1操作
            _mm_storeu_ps(A[k] + j, va); // 把va中的值写回A[k][j]~A[k][j+3]中
        }
        for (; j < NUM; j++)
            A[k][j] /= A[k][k];
        A[k][k] = 1.0; // A[k][k]=1
        for (int i = k + 1; i < NUM; i++) {
            ...
            for (j = k + 1; j + 8 <= NUM; j += 8) {
                __m128 vaij = _mm_loadu_ps(A[i] + j); // 把A[i][j]~A[i][j+7]装入一个SSE变量vaij中
                __m128 vakj = _mm_loadu_ps(A[k] + j); // 把A[k][j]~A[k][j+7]装入一个SSE变量vakj中
                __m128 vx = _mm_mul_ps(vaik, vakj); // 把vaik和vakj中的值取出来相乘
                vaij = _mm_sub_ps(vaij, vx); // 执行vaij-vx操作
                _mm_storeu_ps(A[i] + j, vaij); // 把vaij中的值写回A[i][j]~A[i][j+7]中
            }
            for (; j < NUM; j++)
