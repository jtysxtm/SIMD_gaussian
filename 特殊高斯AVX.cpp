...
void Xor(bitset& b1, bitset& b2) //逐位进行异或操作
{
	...
	for (; i + 8 <= n; i += 8)
	{
		__m256i v1 = _mm256_loadu_si256((__m256i*) & b1.bits[i]);
		__m256i v2 = _mm256_loadu_si256((__m256i*) & b2.bits[i]);
		__m256i result = _mm256_xor_si256(v1, v2);
		_mm256_storeu_si256((__m256i*) & b1.bits[i], result);
	}
